...
<script>
function generateRegistro() {
  let tableHTML = `<table border="1" style="border-collapse:collapse; width:100%">`;
  
  tableHTML += `
    <tr>
      <th style="text-align:center;">
        <div><strong>Blocco</strong></div>
        <div><strong>Caldaia</strong></div>
        <div style="margin-top:5px;">
          <button onclick="resetColumn('caldaia')">Reset</button>
        </div>
      </th>
      <th style="text-align:center;">
        <div><strong>Pastiglie </strong></div>
        <div><strong>Zanzare</strong></div>
        <div style="margin-top:5px;">
          <button onclick="resetColumn('zanzare')">Reset</button>
        </div>
      </th>
    </tr>
  `;
  ...
  for (let i = 0; i < 15; i++) {
    const id1 = `registro-caldaia-${i}`;
    const id2 = `registro-zanzare-${i}`;

    const val1 = localStorage.getItem(id1) || "";
    const val2 = localStorage.getItem(id2) || "";

    tableHTML += `
      <tr>
        <td><input type="date" id="${id1}" value="${val1}"></td>
        <td><input type="date" id="${id2}" value="${val2}"></td>
      </tr>
    `;
  }

  tableHTML += `</table>`;
  document.getElementById("registro-container").innerHTML = tableHTML;
...
// Reset a whole column
function resetColumn(type) {
  for (let i = 0; i < 15; i++) {
    const id = `registro-${type}-${i}`;
    localStorage.removeItem(id);

    // Clear visible input if present
    const el = document.getElementById(id);
    if (el) el.value = "";
  }
}

document.addEventListener("DOMContentLoaded", generateRegistro);
</script>
