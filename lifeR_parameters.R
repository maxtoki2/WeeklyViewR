...
param_mensa_pagine <- c(1, 2)
param_url_mensa_base <- "https://www.comune.sassomarconi.bologna.it/myportal/C_G972/api/content/download"
param_url_mensa_doc_id <- "6aa8fc85ff9f23009973bf24" # find the new ones at https://www.comune.sassomarconi.bologna.it/servizi/educazione-formazione/servizio-mensa-scolastica

param_url_mensa <- glue("{param_url_mensa_base}?id={param_url_mensa_doc_id}")

param_id_medici <- c("boschi" = 21086, "zatti" = 18624, "alberghini" = 28935)


lista_esclusione <- c("prepartita", "postpartita", "approfondimento", "news", "presentazione", "test", "anatomy", "notebook", "studio", "debata", "Baráž", "conferenza")
...
sky_parameters <- list(
  keywords = tibble::tribble(
    ...
    , "Baseball", " ", lista_esclusione, "baseball.png"
  )
  , valid_hours = 7:22
)

dazn_parameters <- list(
  subscriptions = "base_dazn"
  , sport_mapping = tribble(
    ~ Title, ~Sport
